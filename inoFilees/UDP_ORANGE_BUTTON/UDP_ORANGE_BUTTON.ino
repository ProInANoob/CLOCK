...
void WiFiEvent(WiFiEvent_t event) {
  ...
  switch (event) {
    ...
    case ARDUINO_EVENT_WIFI_STA_GOT_IP:
      ...
      //udp.beginMulticast("224.1.1.1", udpRecieve);
      break;
    ...
    case ARDUINO_EVENT_WIFI_AP_STAIPASSIGNED:   Serial.println("Assigned IP address to client");
    deviceCount++;
...
void setup() {
  ...
  udp.beginMulticast(IPAddress(224, 1, 1, 1), 5009); 
...
void loop() {
  ...
  if (len > 0) {
    ...
    //Serial.print(rec_data.color, HEX);
